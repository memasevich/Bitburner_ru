  ...
  const saveToCloud = debounce(
    async (save) => {
      ...
      try {
        ...
        log.error(error);
        utils.writeToast(window, "Не удалось сохранить игру в Steam Cloud.", "error", 5000);
      }
    },
  ...
  const saveToDisk = debounce(
    async (save, fileName) => {
      ...
      try {
        ...
        log.error(error);
        utils.writeToast(window, "Не удалось сохранить игру на диск.", "error", 5000);
      }
    },
...
app.on("ready", async () => {
  ...
  if (!gotTheLock) {
    await dialog.showMessageBox(undefined, {
      ...
      detail: "Bitburner уже запущен. Не запускайте игру несколько раз одновременно.",
      type: "error",
      buttons: ["ОК"],
    });
    app.quit();
  ...
  if (process.argv.includes("--export-save")) {
    ...
    if (global.steamworksError) {
      await dialog.showMessageBox(window, {
        title: "Bitburner",
        message: "Не удалось подключиться к Steam",
        detail: `${global.steamworksError.message}\n\nВы не сможете получать достижения, пока не устраните эту проблему и не перезапустите игру.`,
        type: "warning",
        buttons: ["ОК"],
      });
    }
