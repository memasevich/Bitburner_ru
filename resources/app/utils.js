...
function promptForReload(window) {
  ...
  dialog
    .showMessageBox({
      type: "error",
      title: "Bitburner > Приложение не отвечает",
      message: "Приложение не отвечает. Возможно, в одном из ваших скриптов возник бесконечный цикл.",
      detail:
        " Вы не забыли добавить ns.sleep(x)?\n\n" +
        "Приложение будет перезапущено. Завершить все запущенные скрипты?",
      buttons: ["Перезапустить", "Отмена"],
      defaultId: 0,
      checkboxLabel: "Завершить все запущенные скрипты",
